...
import { ArrowUpRight, Camera, ImagePlus, LockKeyhole, ScanFace, ShieldCheck, Sparkles, X } from 'lucide-react'
import { Link, useOutletContext } from 'react-router-dom'
import { collectPublishedPortraits, loadFaceSearchModels, searchPublishedFaces } from '../services/faceSearchService.js'
...
export function UserFaceSearchPage() {
  ...
  useEffect(() => {
    let active = true
    loadFaceSearchModels()
      .then(() => { if (active) setStatus('ready') })
      .catch(() => {
        if (active) {
          setStatus('error')
          setError('Face search could not start. Refresh the page and check your connection.')
        }
      })
    return () => { active = false }
  }, [])
  ...
  const runSearch = async () => {
    ...
    setResults(null)
    try {
      setResults(await searchPublishedFaces({ file, yearbooks: content.yearbooks, onProgress: setProgress }))
      setStatus('ready')
    } catch (searchError) {
    ...
    <div className="user-route-page user-face-search-page">
      <section className="face-search-hero">
        <div className="face-search-heading">
          ...
          <h1>Find a face in the <em>yearbook.</em></h1>
          <p>Choose one clear photo. GradBook will privately compare it with portraits an administrator approved for active yearbooks and show possible matches.</p>
      ...
      <section className="face-search-workspace" aria-labelledby="face-search-title">
        <div className="face-search-upload-panel">
          <div className="face-search-panel-title"><span><Camera size={18} /> STEP 1</span><h2 id="face-search-title">Choose a search photo</h2><p>Use a front-facing image with one person, even lighting, and a visible face.</p></div>
          ...
          <div className="face-search-actions">
            ...
            <button className="face-search-primary" type="button" disabled={!file || status === 'loading' || status === 'searching' || !portraitCount} onClick={runSearch}>
              <ScanFace size={18} />{status === 'loading' ? 'Preparing secure search…' : status === 'searching' ? `Comparing ${progressPercent}%` : 'Search approved portraits'}
            </button>
          </div>
        ...
        <div className="face-search-results-panel">
          <div className="face-search-panel-title"><span><Sparkles size={18} /> STEP 2</span><h2>Possible matches</h2><p>Face similarity is a search aid. Check the portrait and yearbook details before deciding it is the same person.</p></div>
          ...
          {results?.matches.length > 0 && <div className="face-search-results">
            {results.matches.map((match, index) => <article key={`${match.yearbookId}-${match.id || match.name}`} className="face-search-result">
              ...
              <div><span>{index === 0 ? 'CLOSEST POSSIBLE MATCH' : 'OTHER POSSIBLE MATCH'}</span><h3>{match.name}</h3><p>{[match.strand, match.section, match.schoolYear].filter(Boolean).join(' · ')}</p><small>Similarity score {match.similarity}/100</small><Link to={`/community/yearbooks/${match.yearbookId}`}>Open published yearbook <ArrowUpRight size={14} /></Link></div>
            </article>)}
          </div>}
        </div>
      </section>
